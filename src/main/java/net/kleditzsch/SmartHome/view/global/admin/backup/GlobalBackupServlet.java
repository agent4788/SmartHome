...
public class GlobalBackupServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        model.with("calendarBackups", backups.get(BackupEditor.Module.Calendar).stream().filter(file -> !file.getFileName().contains("auto_")).sorted(Comparator.comparing(BackupFile::getCreationDateTime)).collect(Collectors.toList()));
        ...
        model.with("musicBackups", backups.get(BackupEditor.Module.Music).stream().filter(file -> !file.getFileName().contains("auto_")).sorted(Comparator.comparing(BackupFile::getCreationDateTime)).collect(Collectors.toList()));
        model.with("pictureBackups", backups.get(BackupEditor.Module.Picture).stream().filter(file -> !file.getFileName().contains("auto_")).sorted(Comparator.comparing(BackupFile::getCreationDateTime)).collect(Collectors.toList()));
        ...
        model.with("autoCalendarBackups", backups.get(BackupEditor.Module.Calendar).stream().filter(file -> file.getFileName().contains("auto_")).sorted(Comparator.comparing(BackupFile::getCreationDateTime)).collect(Collectors.toList()));
        ...
        model.with("autoMusicBackups", backups.get(BackupEditor.Module.Music).stream().filter(file -> file.getFileName().contains("auto_")).sorted(Comparator.comparing(BackupFile::getCreationDateTime)).collect(Collectors.toList()));
        model.with("autoPictureBackups", backups.get(BackupEditor.Module.Picture).stream().filter(file -> file.getFileName().contains("auto_")).sorted(Comparator.comparing(BackupFile::getCreationDateTime)).collect(Collectors.toList()));
